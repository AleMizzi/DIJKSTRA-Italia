...
int prendi_source(int n_nodi, vector<string> vect_citta) {
	...
	while (controllo_presenza_file(rad) == false ) //non c'� la citt�
	{
		cout << "Citta non fa parte delle citt� nel database!" << endl;
		cin.clear();
		cin.ignore(256, '\n');
		cin >> rad;
	}
	while (controllo_presenza_vettore(rad , vect_citta) == false) //non c'� la citt�
	{
		cout << "Citta non fa parte delle citt� che hai selezionato!" << endl;
		cin.clear();
		cin.ignore(256, '\n');
		cin >> rad;
	}
...
int prendi_source_nodi(int n_nodi, vector<string> vect_nodi) {
	...
	while (controllo_presenza_vettore(rad, vect_nodi) == false) //non c'� la citt�
	{
...
int controllo_dei_nodi_vicini(vector<int> distanze, vector<bool> sono_stati_visitati, int n_nodi) {
	...
	for (int i = 0; i < n_nodi; i++)
	{
		if (!sono_stati_visitati[i] && distanze[i] <= valoremin)
			...
			//minima distanza che non � nel vect bool dei visitati
		{
...
void dijkstra() {
	...
	cin >> r;
	while (r != "citta" && r != "nodi") //non c'� la citt�
	{
	...
	if (r == "citta") {
		...
		if (risp == 1)
		{
			...
			algoritmo(graph, source, graph.size(), vect_citta_aggiornato,true, vect_citta); // true � il flag per i print differenziale, vect citta � l'originale per il confronto
		}
